...
def test_create_multiple_expenses_and_list():
    ...
    services = service.list_expenses()
    assert services[0].title == "Pan"
    assert services[1].title == "Leche"
...
def test_remove_expense_reduces_total():
    ...
    """
    service = create_service()
    service.create_expense("Libro", 3, "Libreria", date.today())
    service.create_expense("Revista", 14, "Supermercado", date.today())
    service.remove_expense(1)
    services = service.list_expenses()
    assert len(services) == 1
    assert services[0].title == "Revista"
...
def test_update_expense_partial_fields():
    ...
    services = service.list_expenses()
    assert services[0].title == "Camiseta"
    assert services[0].amount == 18
    assert services[0].description == "Ropa"
...
def test_total_amount_after_removal():
    ...
    """
    service = create_service()
    service.create_expense("Cursos", 30, "Gasto1", date.today())
    service.create_expense("Internet", 25, "Gasto1", date.today())
