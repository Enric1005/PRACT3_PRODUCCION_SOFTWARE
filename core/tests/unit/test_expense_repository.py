...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense1 = Expense(13, "dkdjsak", 14, "kaskdals", date.today())
    expense2 = Expense(2, "dkdjsak", 5, "kaskdal", date.today())
    repo.save(expense1)
    repo.save(expense2)
    assert repo.get_by_id(13).amount == 14
...
def test_get_by_id_returns_none_if_not_found():
    ...
    repo.save(expense1)
    repo.save(expense2)
    assert repo.get_by_id(213) is None
